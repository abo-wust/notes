...
在linux当中，socket包括两类：使用网络的socket，称为Berkeley socket；

不使用网络的socket，称为Unix domain socket。两者API结构很像，所以容易被混淆，但是其实不是一个东西。
...
![](https://picd.zhimg.com/80/v2-d397c683c0609ec73371a37aaebf3376_720w.jpg?source=1940ef5c)
...
![](https://pic1.zhimg.com/80/v2-8319e4008afe2534d41ba5ad29bb7447_720w.jpg?source=1940ef5c)
...
* 第二是构造一个` struct msghdr `对象，把用户传入的数据，比如` buffer`地址、数据长度啥的，统统都装进去. 剩下的事情就交给下一层，协议栈里的函数` inet_sendmsg `了，其中` inet_sendmsg `函数的地址是通过` socket `内核对象里的` ops `成员找到的。大致流程如图。

![](https://pica.zhimg.com/80/v2-b65fdb6a4828626631b6fe5cb0b1f765_720w.jpg?source=1940ef5c)
...
![](https://picd.zhimg.com/80/v2-f1c2ab0d2a2223594f507993066ce273_720w.jpg?source=1940ef5c)
...
![](https://picd.zhimg.com/80/v2-bf038e44dfd6923f4f78057efbf33181_720w.jpg?source=1940ef5c)
...
![](https://picd.zhimg.com/80/v2-8319e4008afe2534d41ba5ad29bb7447_720w.jpg?source=1940ef5c)

相对比本机网络 IO 通信过程上，它的工作过程要清爽许多。其中 127.0.0.1 工作过程如下图。

![](https://pic1.zhimg.com/80/v2-1893a0e2865e14a61a5b2b443bdda571_720w.jpg?source=1940ef5c)


也对比了 UDP 和 TCP 两种方式下的延迟和性能指标。在包体不大于 1KB 的时候，UDS 的性能大约是 TCP 的两倍多。所以，在本机网络 IO 的场景下，如果对性能敏感，可考虑使用 Unix Domain Socket。
